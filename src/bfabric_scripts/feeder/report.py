...
def report_resource(client: Bfabric, resource_id: int) -> None:
    ...
    if filename.is_file():
        ...
        client.save("resource", {"id": resource_id, "size": filesize, "status": "available", "filechecksum": checksum})
    else:
        client.save("resource", {"id": resource_id, "status": "failed"})
