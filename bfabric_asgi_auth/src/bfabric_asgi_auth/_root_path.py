...
- ``_mount_prefix``: read ``scope["root_path"]`` with trailing slash and empty-ish
  values normalized away.
- ``_strip_root_path``: return the in-app path (mount prefix removed, boundary-aware).
- ``_prepend_root_path``: prepend the mount prefix to a root-relative URL, leaving
  other URL shapes alone.

The strip/prepend pair satisfies a round-trip invariant:
``_strip_root_path({"path": _prepend_root_path(url, scope), **scope}) == url`` for
...
def _is_root_relative(url: str) -> bool:
...
def _prepend_root_path(url: str, scope: Scope) -> str:
    ...
    :func:`_is_root_relative` for the full list of shapes that skip the prefix.
    """
    if not _is_root_relative(url):
        return url
    return _mount_prefix(scope) + url
