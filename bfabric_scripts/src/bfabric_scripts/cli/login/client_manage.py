...
def _persist_rotated_credentials(result: dict[str, object], env_name: str, config_file: Path) -> None:
    ...
    Routed through :func:`merge_auth_owned_keys`: the writer replaces every auth-owned key at once, so
    writing only the rotated ones would drop whichever were not rotated this time.
    ...
    if not rotated:
        return
    merged = stored | rotated
    write_environment_to_config(config_file, env_name, merged, set_default=False)
...
def cmd_auth_client_delete(
    *,
    ...
    remaining = merge_auth_owned_keys(config_file, env_name, {})
    for key in ("client_secret", "registration_access_token", "registration_client_uri"):
        _ = remaining.pop(key, None)
    write_environment_to_config(config_file, env_name, remaining, set_default=False)
