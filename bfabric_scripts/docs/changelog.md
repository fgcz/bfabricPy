...
- `bfabric-cli workunit upload --link-duplicates` — register a file whose content already exists in the container as a link to that resource, instead of skipping the file. The workunit then has a resource for every file uploaded, without re-transferring content B-Fabric already stores. The final summary line reports the number linked when any were.
