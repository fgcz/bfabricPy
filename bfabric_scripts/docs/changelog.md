...
- `auth list` groups environments by instance and shows the account each cached token belongs to, its scope, and its expiry — which is what tells two logins on one instance apart. `auth status` adds the account and reports the granted scope separately when it differs from the requested one. Both annotate *why* an environment is the active one, since `BFABRICPY_CONFIG_ENV` silently outranks the configured default. The account is read from the access token locally (display only, no signature verification), so no extra scope is needed; a non-JWT token degrades to "unknown".
...
- Base URLs are normalised offline (scheme defaulted, host lowercased, trailing slash dropped, bare known hosts expanded, non-http(s) rejected) and pre-flighted against OIDC discovery *before* the browser opens, correcting a missing `/bfabric` segment. Advisory only: an unconfirmed URL warns and proceeds, since a discovery miss also covers an instance that publishes no metadata.
...
- Internal: login handlers normalised to `cmd_auth_*` (was a mix of `cmd_auth_*` and `cmd_login_*`); shared resolution in `cli/login/_common.py` plus new `_instances.py` (known-host list) and `_identity.py` (token claim reading); first test coverage of the `auth` command wiring.
