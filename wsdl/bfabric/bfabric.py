...
if [ -x /usr/bin/mutt ];
then
    qstat -ext -xml > /tmp/qstat.$JOB_ID.xml
    ...
    | mutt -s "JOB_ID=$JOB_ID WORKUNIT_ID=$WORKUNIT_ID EXTERNALJOB_ID=$EXTERNALJOB_ID" $EMAIL \
        -a /tmp/$JOB_ID.bash /tmp/qstat.$JOB_ID.xml /tmp/config.$JOB_ID.yaml 
fi
# exit 0
...
then
    ...
    # has do be done on success
     mutt -s "JOB_ID=$JOB_ID WORKUNIT_ID=$WORKUNIT_ID EXTERNALJOB_ID=$EXTERNALJOB_ID DONE" $EMAIL < /dev/null
     /home/bfabric/.python/fgcz_bfabric_setExternalJobStatus_done.py $EXTERNALJOB_ID
    echo $?
else
