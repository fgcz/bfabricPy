...
from bfabric_scripts.cli.login.manage import cmd_auth_remove
...
class TestCmdAuthRemove:
    def test_removes_oauth_env_and_clears_cache(self, tmp_path, capsys, mocker):
        ...
        cmd_auth_remove("PROD", config_file=config_file, no_confirm=True)
    ...
    def test_removing_default_env_points_to_auth_activate(self, tmp_path, capsys, mocker):
        config_file = tmp_path / "config.yml"
        _write_config(config_file, default="PROD")
        ...
        assert "auth activate" in output
        data = yaml.safe_load(config_file.read_text())
        assert data.get("GENERAL", {}).get("default_config") is None
    ...
    def test_removes_non_oauth_env_without_touching_cache(self, tmp_path, capsys, mocker):
        ...
        cmd_auth_remove("TEST", config_file=config_file, no_confirm=True)
    ...
    def test_confirmation_declined_keeps_env(self, tmp_path, capsys, mocker):
        ...
        cmd_auth_remove("PROD", config_file=config_file)
    ...
    def test_interactive_picker_selects_env(self, tmp_path, mocker):
        ...
        cmd_auth_remove(config_file=config_file)
    ...
    def test_unknown_env(self, tmp_path, capsys):
        config_file = tmp_path / "config.yml"
        _write_config(config_file, default="PROD")
        cmd_auth_remove("NOPE", config_file=config_file, no_confirm=True)
        output = capsys.readouterr().out
