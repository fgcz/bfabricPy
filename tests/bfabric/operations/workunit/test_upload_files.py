...
def test_happy_path(mock_client, rest, mock_collect, mock_send):
...
def test_happy_path_upload_records(mock_client, rest, mock_collect, mock_send):
...
# --- scenario 2: all duplicates skipped -----------------------------------------------------------


def test_all_duplicates_skipped(mock_client, rest, mock_collect, mock_send):
    mock_collect.return_value = _file_infos("a.txt", "b.txt")
...
# --- scenario 3: force=True skips the duplicate check ---------------------------------------------


def test_force_skips_duplicate_check(mock_client, rest, mock_collect, mock_send):
    mock_collect.return_value = _file_infos("a.txt", "b.txt")
    rest.create_resources.return_value = _created("a.txt", "b.txt")
    rest.get_upload_token.return_value = UploadTokenResult(token="tok", tus_endpoint="https://tus/")
...
# --- scenario 4: partial failure (one transfer fails) ---------------------------------------------


def test_partial_failure(mocker, mock_client, rest, mock_collect, mock_send):
    mock_collect.return_value = _file_infos("a.txt", "b.txt")
    rest.check_duplicates.return_value = _dupes(**{"a.txt": "upload", "b.txt": "upload"})
    rest.create_resources.return_value = _created("a.txt", "b.txt")
    rest.get_upload_token.return_value = UploadTokenResult(token="tok", tus_endpoint="https://tus/")
...
# --- scenario 5: all transfers fail ---------------------------------------------------------------


def test_all_transfers_fail(mock_client, rest, mock_collect, mock_send):
    mock_collect.return_value = _file_infos("a.txt", "b.txt")
    rest.check_duplicates.return_value = _dupes(**{"a.txt": "upload", "b.txt": "upload"})
    rest.create_resources.return_value = _created("a.txt", "b.txt")
    rest.get_upload_token.return_value = UploadTokenResult(token="tok", tus_endpoint="https://tus/")
...
# --- scenario 6: setup failure marks failed + reraises --------------------------------------------


def test_setup_failure_marks_failed_and_reraises(mock_client, rest, mock_collect, mock_send):
    mock_collect.return_value = _file_infos("a.txt", "b.txt")
    rest.check_duplicates.return_value = _dupes(**{"a.txt": "upload", "b.txt": "upload"})
...
# --- scenario 7: resource pairing mismatch --------------------------------------------------------


def test_resource_pairing_mismatch_marks_failed_and_reraises(mock_client, rest, mock_collect, mock_send):
    mock_collect.return_value = _file_infos("a.txt", "b.txt")
    rest.check_duplicates.return_value = _dupes(**{"a.txt": "upload", "b.txt": "upload"})
...
# --- scenario 8: audit attributes are written -----------------------------------------------------


def test_audit_attributes_written(mock_client, rest, mock_collect, mock_send):
    mock_collect.return_value = _file_infos("a.txt")
    rest.check_duplicates.return_value = _dupes(**{"a.txt": "upload"})
    rest.create_resources.return_value = _created("a.txt")
    rest.get_upload_token.return_value = UploadTokenResult(token="tok", tus_endpoint="https://tus/")
...
# --- scenario 9: progress callbacks (on_start / on_file_done) -------------------------------------


def test_on_start_fires_once_with_count_and_total_bytes(mocker, mock_client, rest, mock_collect, mock_send):
...
def test_on_start_not_fired_when_all_skipped(mocker, mock_client, rest, mock_collect, mock_send):
    mock_collect.return_value = _file_infos("a.txt")
...
def test_on_file_done_fires_per_file_on_success(mocker, mock_client, rest, mock_collect, mock_send):
    mock_collect.return_value = _file_infos("a.txt", "b.txt")
    rest.check_duplicates.return_value = _dupes(**{"a.txt": "upload", "b.txt": "upload"})
    rest.create_resources.return_value = _created("a.txt", "b.txt")
    rest.get_upload_token.return_value = UploadTokenResult(token="tok", tus_endpoint="https://tus/")
...
def test_on_file_done_reports_failure(mocker, mock_client, rest, mock_collect, mock_send):
    mock_collect.return_value = _file_infos("a.txt", "b.txt")
    rest.check_duplicates.return_value = _dupes(**{"a.txt": "upload", "b.txt": "upload"})
    rest.create_resources.return_value = _created("a.txt", "b.txt")
    rest.get_upload_token.return_value = UploadTokenResult(token="tok", tus_endpoint="https://tus/")
...
def test_non_oauth_client_rejected_before_any_work(mock_client, mock_collect, mock_send):
...
def test_missing_tus_extra_fails_fast(mocker, mock_client, mock_collect, mock_send):
    mocker.patch(
...
def test_missing_tus_scope_fails_before_workunit_creation(mocker, mock_client, rest, mock_collect, mock_send):
...
def test_check_duplicates_missing_verdict_rejected(mock_client, rest, mock_collect, mock_send):
    mock_collect.return_value = _file_infos("a.txt", "b.txt")
...
def test_check_duplicates_link_verdict_rejected(mock_client, rest, mock_collect, mock_send):
...
def test_check_duplicates_unknown_action_rejected(mock_client, rest, mock_collect, mock_send):
...
def test_duplicate_resource_names_rejected(mock_client, rest, mock_collect, mock_send):
...
# --- scenario 10: reuse an existing workunit (--workunit-id) ---------------------------------------


def test_reuse_existing_workunit_does_not_create(mock_client, rest, mock_collect, mock_send):
...
def test_reuse_workunit_not_found_raises(mock_client, rest, mock_collect, mock_send):
...
def test_reuse_all_duplicates_skipped_reports_workunit_id(mock_client, rest, mock_collect, mock_send):
...
def test_reuse_all_transfers_fail_does_not_mark_failed(mock_client, rest, mock_collect, mock_send):
    mock_client.read.return_value = [{"container": {"id": 777}}]
    mock_collect.return_value = _file_infos("a.txt")
    rest.check_duplicates.return_value = _dupes(**{"a.txt": "upload"})
    rest.create_resources.return_value = _created("a.txt")
...
def test_reuse_setup_failure_does_not_mark_failed_and_reraises(mock_client, rest, mock_collect, mock_send):
    mock_client.read.return_value = [{"container": {"id": 777}}]
    mock_collect.return_value = _file_infos("a.txt")
    rest.check_duplicates.return_value = _dupes(**{"a.txt": "upload"})
...
# --- scenario 11: job tracking (--track-job) ------------------------------------------------------


def test_track_job_creates_job_and_threads_id(mock_client, rest, mock_collect, mock_send):
...
def test_track_job_off_by_default_creates_no_job(mock_client, rest, mock_collect, mock_send):
    mock_collect.return_value = _file_infos("a.txt")
    rest.check_duplicates.return_value = _dupes(**{"a.txt": "upload"})
    rest.create_resources.return_value = _created("a.txt")
    rest.get_upload_token.return_value = UploadTokenResult(token="tok", tus_endpoint="https://tus/")
...
def test_track_job_on_reuse_path_creates_job_without_touching_workunit(mock_client, rest, mock_collect, mock_send):
...
# --- scenario 12: UploadFilesParams target validation ---------------------------------------------


def test_params_workunit_id_and_name_mutually_exclusive():
    with pytest.raises(ValidationError, match="mutually exclusive"):
        UploadFilesParams(workunit_id=1, workunit_name="x")


def test_params_requires_container_and_application_without_id():
...
def test_params_workunit_id_alone_is_valid():
