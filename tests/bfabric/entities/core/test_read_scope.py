...
class TestAmbientContext:
    def test_get_read_scope_raises_when_none_active(self):
    ...
    def test_nesting_merges_by_instance(self, client_a, client_b):
        ...
        with outer, inner:
            ...
            assert get_read_scope() is inner
        assert True  # both contexts exited cleanly

    def test_reentry_same_scope_restores_cleanly(self, client_a):
        ...
        # scope object. Both exits must succeed and the context must be fully cleared.
        scope = ReadScope(client_a)
        with scope:
            with scope:
                assert get_read_scope() is scope
...
class TestConstruction:
    def test_single_client(self, client_a):
        assert ReadScope(client_a).instances == [INSTANCE_A]

    def test_multiple_clients(self, client_a, client_b):
        assert set(ReadScope([client_a, client_b]).instances) == {INSTANCE_A, INSTANCE_B}
    ...
    def test_reader_for_unknown_instance_raises(self, client_a):
        with pytest.raises(LookupError, match="No B-Fabric connection registered"):
            ReadScope(client_a)._reader_for(INSTANCE_B)
    ...
    def test_default_instance_ambiguous_raises(self, client_a, client_b):
        with pytest.raises(LookupError, match="pass bfabric_instance="):
            ReadScope([client_a, client_b])._default_instance()

    def test_read_only_no_write_methods(self, client_a):
        scope = ReadScope(client_a)
        assert not hasattr(scope, "save")
        assert not hasattr(scope, "delete")


class TestRouting:
    def test_read_uris_routes_each_instance_to_its_reader(self, mocker, client_a, client_b):
        scope = ReadScope([client_a, client_b])
        ...
        result = scope.read_uris([uri_a, uri_b])
    ...
    def test_query_routes_to_named_instance(self, mocker, client_a, client_b):
        ...
        result = scope.query("workunit", {"status": "processing"}, bfabric_instance=INSTANCE_B)
    ...
    def test_read_id_uses_default_instance(self, mocker, client_a):
        ...
        assert scope.read_id("workunit", 7) is entity
