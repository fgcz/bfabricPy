...
def test_read_reference_when_result_found(bfabric_instance, mocker):
    """Test read_reference when exactly one result is found."""
    mock_auth = mocker.MagicMock(name="mock_auth")
    bfabric_instance._auth = mock_auth

    mock_read = mocker.patch.object(bfabric_instance, "read")
    mock_result = [{"id": 123, "name": "test_entity"}]
    mock_read.return_value = mock_result

    reference = {"classname": "sample", "id": 123}
    result = bfabric_instance.read_reference(reference)

    assert result == {"id": 123, "name": "test_entity"}
    mock_read.assert_called_once_with("sample", {"id": 123}, max_results=1, check=True)


def test_read_reference_when_no_result_found(bfabric_instance, mocker):
    ...
    mock_read.assert_called_once_with("sample", {"id": 999}, max_results=1, check=True)


def test_read_reference_when_multiple_results_found(bfabric_instance, mocker):
...
def test_read_reference_with_check_false(bfabric_instance, mocker):
    ...
    result = bfabric_instance.read_reference(reference, check=False)

    assert result == {"id": 123, "name": "test_entity"}
    mock_read.assert_called_once_with("workunit", {"id": 456}, max_results=1, check=False)
