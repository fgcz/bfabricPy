...
class BfabricFunctionalTestCase(unittest.TestCase):
    ...
    def test_wrappercreator_submitter(self):
        ...
        try:
            ...
            self.assertEqual(res[0].status, 'done', f'set externaljob id={externaljobid_wc} of wrapper creator failed.')
        except:
        ...
        try:
            ...
            self.assertEqual(res[0].status, 'done', f'submitter externaljob with id={externaljobid_submitter} failed.')
        except:
        ...
        try:
            ...
            logging.info(f"Checking if WORKUNIT's externaljob with id={externaljobid_yaml_workunit} was set to 'done'")
            res = B.read_object('externaljob', {'id': externaljobid_yaml_workunit, 'status':'DONE'})
            self.assertEqual(res[0].status, 'done', f'yaml_workunit_externaljob with id={externaljobid_yaml_workunit} failed.')
        except:
