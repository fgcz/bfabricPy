...
def add_dataset(containerid):
    ...
    try:
        obj = {}
        obj["name"] = (
            "autogenerated dataset by http://fgcz-s-028.uzh.ch:8080/queue_generator/"
        )
        obj["containerid"] = containerid
        obj["attribute"] = [
            {"name": "File Name", "position": 1, "type": "String"},
...
def get_all_sample(containerid):
    ...
    rv = list(
        map(
            lambda p: bfapp.read_object(
                endpoint="sample", obj={"containerid": containerid}, page=p
            ),
            range(1, 10),
        )
    )
    ...
    try:
        ...
        for annotationId in filter(
            ...
            set(
                map(
                    lambda x: x.groupingvar._id if "groupingvar" in x else None, samples
                )
            ),
        ):
            print(annotationId)
            annotation = bfapp.read_object(
                endpoint="annotation", obj={"id": annotationId}
            )
