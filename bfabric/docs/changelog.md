...
- New `bfabric.transfer` layer for moving files to and from B-Fabric, in a single namespace:
    ...
    - B-Fabric domain binding: `resource_sources` / `ssh_source` / `http_source` (Resource → download sources), `UploadRestClient` + `tus_sink_for_resource` (`/rest/upload/*` → tus upload sink), `token_provider`, and fail-fast OAuth scope checks (`check_upload_scope` / `check_download_scope`).
