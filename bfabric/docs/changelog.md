...
- The OAuth module moved from `bfabric._oauth` to `bfabric.oauth`, which exports `WebappClient`, `UrlTokenContext`, `pkce_login`, `device_code_login`, `OAuthCredentialProvider`, `register_client`, `register_webapp`, `TokenCache` and `compute_token_cache_path`. Its submodules are private and the exports stay provisional while the OAuth work is in flight — they may change without a deprecation cycle.
