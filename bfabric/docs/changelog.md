...
- `Bfabric.current_login` — resolve the authenticated user's login across all auth modes (OAuth: the access-token JWT `subject`; password/webapp token: the configured login). Raises `ValueError` for opaque Personal Access Tokens (`connect_pat`), whose login cannot be determined locally.
- `User.current(client)` — resolve the logged-in user as a `User` entity (via `current_login` → `find_by_login`). Returns `None` when no user record matches the login (e.g. an OAuth service account).
