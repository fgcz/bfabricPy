...
- `UploadSummary` gains `linked` (a `list[FileUpload]`) and `linked_count`, reported separately from `uploaded`/`skipped`: a linked file *has* a resource but transferred no bytes, whereas a skipped one has no resource at all. A workunit whose files were all linked now completes instead of being marked failed by the "nothing uploaded" check.
