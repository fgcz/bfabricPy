...
- The version banner logged on every `Bfabric` construction is now emitted at `DEBUG` instead of `INFO`, so library, server, and test contexts stay quiet by default (CLI scripts can still surface it via `BFABRICPY_LOG_LEVEL=DEBUG`).
