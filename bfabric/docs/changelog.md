...
- `write_environment_to_config` now **merges** into an existing environment instead of replacing it, so unrelated keys (`application_ids`, `engine`, `job_notification_emails`, hand-written extras) survive a re-login. Auth-owned keys (`login`, `password`, `pat`, `auth_method`, `client_id`, `scope`) are replaced wholesale, so a stale `pat` cannot outlive the auth method that wrote it. Round-trip validation now runs on the merged environment, still before any filesystem write.
...
- New `bfabric._oauth.discovery` — `fetch_discovery_document` and `resolve_base_url` for OIDC discovery lookups, used as a pre-flight that catches a mistyped base URL (typically a missing `/bfabric` segment) before a browser flow runs. Both fail open: a miss is reported, never raised, since it also covers an instance that publishes no discovery document.
