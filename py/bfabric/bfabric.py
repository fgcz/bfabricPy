...
$Id: bfabric.py 2641 2017-02-24 13:46:11Z cpanse $
...
class Bfabric(object):
    ...
    def __init__(self, login=None, password=None, externaljobid=None, bfabricrc=None):
        if bfabricrc:
...
class BfabricSubmitter(BfabricExternalJob, gridengine.GridEngine):
    ...
    def __init__(self, login=None, password=None, externaljobid=None,
                 user='*', queue="PRX@fgcz-c-071", GRIDENGINEROOT='/opt/sge'):
        """
...
cat > /tmp/config.$JOB_ID.yaml <<EOF
{8}
EOF
...
then
    ...
    | mutt -s "JOB_ID=$JOB_ID WORKUNIT_ID=$WORKUNIT_ID EXTERNALJOB_ID=$EXTERNALJOB_ID" $EMAIL \
        -a /tmp/$JOB_ID.bash /tmp/config.$JOB_ID.yaml 
fi
# exit 0

# run the application
test -f /tmp/config.$JOB_ID.yaml && {9} /tmp/config.$JOB_ID.yaml

sleep 10

if [ $? -eq 0 ];
then
     ssh fgcz-ms.uzh.ch "fgcz_bfabric_setResourceStatus_available.py $RESSOURCEID_OUTPUT" \
     | mutt -s "JOB_ID=$JOB_ID WORKUNIT_ID=$WORKUNIT_ID EXTERNALJOB_ID=$EXTERNALJOB_ID DONE" $EMAIL 

     fgcz_bfabric_setExternalJobStatus_done.py $EXTERNALJOB_ID
    echo $?
else
    echo "application failed"
     mutt -s "JOB_ID=$JOB_ID WORKUNIT_ID=$WORKUNIT_ID EXTERNALJOB_ID=$EXTERNALJOB_ID failed" $EMAIL < /dev/null
     fgcz_bfabric_setResourceStatus_available.py $RESSOURCEID_STDOUT_STDERR $RESSOURCEID;
    exit 1;
fi

# should be available also as zero byte files
fgcz_bfabric_setResourceStatus_available.py $RESSOURCEID_STDOUT_STDERR
