...
@use_client
def cmd_run_dispatch(*, action: Annotated[ActionDispatch, Parameter(name="*")], client: Bfabric) -> None:
...
@use_client
def cmd_run_inputs(*, action: Annotated[ActionInputs, Parameter(name="*")], client: Bfabric) -> None:
    """Prepares the input files by downloading and generating them (if necessary)."""
    execute(action, client=client)


@use_client
def cmd_run_process(*, action: Annotated[ActionProcess, Parameter(name="*")], client: Bfabric) -> None:
    """Processes a chunk."""
    execute(action, client=client)


@use_client
def cmd_run_outputs(*, action: Annotated[ActionOutputs, Parameter(name="*")], client: Bfabric) -> None:
