...
def _export_env_data(
    config_data: ConfigData | None, app_spec: Path | str, workunit_ref: Path, uv_bin: Path
) -> dict[str, str]:
...
def write_env_data(work_dir: Path, config_data: ConfigData, app: Path | str, workunit: Path, uv_bin: Path) -> None:
    """Writes environment data for `work_dir`."""
    env_data = _export_env_data(config_data=config_data, app_spec=app, workunit_ref=workunit, uv_bin=uv_bin)
