...
- **`engine/`** — `EngineSUDS`, the SOAP transport, over the suds library. It is the only engine; `BfabricClientConfig.engine` is vestigial and a config naming any other value fails to load.
